...
def run_pie_model_general(theta):
    ...
    # Exo-Zodi Parameters
    Nez = theta_dict.get("Nez", THETA_DEFAULTS["Nez"])
...
PRIORS = [
    ...
    smarter.priors.UniformPrior(0.0, 30.0, theta_name = "Nez", theta0 = THETA_DEFAULTS["Nez"]),
]
...
tag = "test_free_allin4_zodi10_fit1"
...
if os.path.exists(data_tag+".npz"):
    ...
    wl, y_binned, y_meas, y_err, XSECS = generate_data(savetag = data_tag, niriss = True, nirspec = True, mirilrs = True,
                                                       Texp=4.0)
...
if __name__ == "__main__":
    ...
    # Run MCMC inference
    run_mcmc(tag, processes = ncpu, nsteps = nsteps, p0 = p0)
