...
def make_stellar_multicomponent(fracs, temp, logMH, logg, database, outfile,
                                write_file = True, return_spectrum = False):
    """
    Make stellar spectrum using pysynphot assuming that the star has a multi-component spectrum.

    Parameters
    ----------
    fracs : list of floats
        fractions to weight each spectral component
    temp : list of float
        temperature
    logMH : list of float
        log stellar metallicity
    ...
    for i, f in enumerate(fracs):
        ...
        # Get single star spectrum
        wave, flux = make_stellar(temp[i], logMH[i], logg[i], database, outfile,
                                     write_file = False, return_spectrum = True)

        if i==0:
            fluxmks = fracs[i] * flux
        else:
            fluxmks += fracs[i] * flux
